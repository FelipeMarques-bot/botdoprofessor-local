...
chamada_bp = Blueprint("chamada", __name__, url_prefix="/api/chamada")


@chamada_bp.route("/from-image", methods=["POST"])
@require_auth
def extract_chamada_from_image():
    """Extract chamada (frequencia de um dia) from an uploaded photo.
    ...
    Returns:
        JSON with extracted chamada array (aluno, situacao, faltas, motivo, descricao).
    """
    from bot.utils.image_chamada_extractor import extract_chamada_from_image as _extract
    from bot.utils.image_grade_extractor import is_available

    if not is_available():
        AuditLog.log(g.current_user.id, "chamada", status="error", details="AI vision not available")
        return jsonify({
    ...
    if result.get("error"):
        AuditLog.log(g.current_user.id, "chamada", status="error", details=result["error"])
        return jsonify(result), 500

    AuditLog.log(
        g.current_user.id, "chamada",
        status="success",
