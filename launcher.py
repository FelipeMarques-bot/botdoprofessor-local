...
APP_FILES = [
    ...
    "lancar_professor_online.py", "aprender_novo_portal.py",
...
def _launch_updater(splash):
    ...
    content = (
        ...
        "echo [%date% %time%] Iniciando updater >>\"%LOG%\"\r\n"
        "set \"TRIES=0\"\r\n"
        ":retry\r\n"
        "if not exist \"%NEWEXE%\" (\r\n"
        "    echo [%date% %time%] Novo exe ausente, reabrindo versao atual >>\"%LOG%\"\r\n"
        "    start \"\" \"%OLDEXE%\"\r\n"
        "    goto done\r\n"
        ")\r\n"
        ...
        "set /a TRIES+=1\r\n"
        "if %TRIES% EQU 1 echo [%date% %time%] Aguardando o programa antigo fechar >>\"%LOG%\"\r\n"
        "if %TRIES% GEQ 300 (\r\n"
        "    echo [%date% %time%] Timeout aguardando fechamento, reabrindo versao atual >>\"%LOG%\"\r\n"
        "    start \"\" \"%OLDEXE%\"\r\n"
        "    goto done\r\n"
        ")\r\n"
        "goto retry\r\n"
        ":launched\r\n"
        "echo [%date% %time%] Substituido - reabrindo automaticamente >>\"%LOG%\"\r\n"
        "for %%A in (\"%OLDEXE%\") do set \"EXENAME=%%~nxA\"\r\n"
        ":reopen\r\n"
        "tasklist /fi \"IMAGENAME eq %EXENAME%\" | find /i \"%EXENAME%\" >nul\r\n"
        "if not errorlevel 1 (\r\n"
        "    echo [%date% %time%] Nova versao ja em execucao >>\"%LOG%\"\r\n"
        "    goto done\r\n"
        ")\r\n"
        ...
        "tasklist /fi \"IMAGENAME eq %EXENAME%\" | find /i \"%EXENAME%\" >nul\r\n"
        "if not errorlevel 1 (\r\n"
        "    echo [%date% %time%] Nova versao iniciada com sucesso >>\"%LOG%\"\r\n"
        "    goto done\r\n"
        ")\r\n"
        "set /a TRIES+=1\r\n"
        "if %TRIES% GEQ 6 (\r\n"
        "    echo [%date% %time%] Falha ao reabrir apos tentativas - abra o BotDoProfessor.exe manualmente >>\"%LOG%\"\r\n"
        "    goto done\r\n"
        ")\r\n"
        "echo [%date% %time%] Reabrindo (tentativa %TRIES%)... >>\"%LOG%\"\r\n"
        "goto reopen\r\n"
        ":done\r\n"
        "del \"%BAT%\" >nul 2>nul\r\n"
        "endlocal\r\n"
